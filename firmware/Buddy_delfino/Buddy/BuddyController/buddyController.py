...
def set_motor_thread():
    global NEW_JOYPOS, leftMotorTargVel, rightMotorTargVel, output_buff, syncError
    while(True):
        if NEW_JOYPOS:
            logging.info( "Sending new Motor Command" )
            logging.info('Left Motor: {}'.format(leftMotorTargVel))
            output_buff.append(BuddyPacket.LEFT_MOTOR)
            output_buff.append(leftMotorTargVel)  
            logging.info('Right Motor: {}'.format(rightMotorTargVel))
            output_buff.append(BuddyPacket.RIGHT_MOTOR)
            output_buff.append(rightMotorTargVel)
            NEW_JOYPOS = False
